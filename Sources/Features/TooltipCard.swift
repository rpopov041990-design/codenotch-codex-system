...
enum UsageFormat {
    static func tokens(_ value: Int?, locale: Locale = L10n.locale) -> String {
        ...
        switch value {
        case 1_000_000_000...:
            return String(format: russian ? "%.2f млрд" : "%.2fB", locale: russian ? locale : Locale(identifier: "en_US_POSIX"),
                          Double(value) / 1_000_000_000)
        case 1_000_000...:
            return String(format: russian ? "%.1f млн" : "%.1fM", locale: russian ? locale : Locale(identifier: "en_US_POSIX"),
                          Double(value) / 1_000_000)
        case 1_000...:
            return String(format: russian ? "%.0f тыс." : "%.0fK", locale: russian ? locale : Locale(identifier: "en_US_POSIX"),
                          Double(value) / 1_000)
        default:
            return "\(value)"
        }
    ...
    static func duration(seconds: Double?, locale: Locale = L10n.locale) -> String {
        ...
        if hours > 0 {
            return remainder == 0 ? L10n.t("\(hours)h", locale: locale) : L10n.t("\(hours)h \(remainder)m", locale: locale)
        }
        return L10n.t("\(minutes)m", locale: locale)
    }

    static func days(_ value: Int?, locale: Locale = L10n.locale) -> String {
        guard let value else { return "—" }
        return L10n.t("\(value)d", locale: locale)
    }
}
