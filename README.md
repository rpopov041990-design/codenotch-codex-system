...
| **Личные токены** | Расход аккаунта Codex за сегодня | Отдельный график за 30 дней; не процент лимита |
...
| **Температура CPU/GPU** | Средняя температура в °C внутри кружка | Среднее доступных тепловых зон Apple M5; при отсутствии датчиков — прочерк |
...
**Анимации показателей.** Кольца и числа плавно меняются, график появляется при наведении, значок токенов пульсирует. Пульсация декоративная и не обозначает работу агента. Настройка macOS «Уменьшение движения» отключает добавленные анимации.
...
Температура: read-only AppleSMC, без sudo и управления вентиляторами. Среднее арифметическое доступных зон CPU/GPU с одинаковым весом каждого датчика, без батареи. Это не температура корпуса и не точное число физических ядер. Карта датчиков ограничена семейством Apple M5; приватный интерфейс macOS может измениться. На других моделях пока выводится прочерк. Источник карты ключей: [Stats](https://github.com/exelban/stats/blob/master/Modules/Sensors/values.swift).

Личные токены: серверная статистика всего аккаунта Codex, а не только этого компьютера или чата. Возможна задержка публикации данных. Серые отметки на графике означают отсутствие данных, а не нулевой расход. [Изменения и проверки от 24.09.2026](docs/macos-panel-2026-09-24.md).
