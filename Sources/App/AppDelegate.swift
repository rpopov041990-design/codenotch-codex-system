...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    // Only the default Codex account is authorized in this local build.
    private let codexProfiles: [CodexProfile] = [.default()]
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // A separate defaults domain; do not import upstream account choices.
        ...
        if ProcessInfo.processInfo.environment["CODENOTCH_DEMO"] == "1" {
            ...
            let store = UsageStore(
                providers: codexProfiles.map { CodexLocalProvider(profile: $0) },
            ...
            let relayConfiguration = relayPreferences.map { values in
                (enabled: false, endpoint: values.1)
            }.eraseToAnyPublisher()
            relayConfiguration
