...
final class NotchViewModel: ObservableObject {
    // Provider API remains separate from the presentation-only system cells.
    var snapshots: [ProviderSnapshot] {
        get { displaySnapshots.filter { !$0.id.hasPrefix("system.") } }
        set {
            displaySnapshots = displaySnapshots.filter { $0.id.hasPrefix("system.") } + newValue
        }
    }
    @Published var displaySnapshots: [ProviderSnapshot] = []
    private(set) var systemMetrics: [SystemMetric] = []
    private var providerReadings: [ProviderSnapshot] = []
    ...
    func startSystemMetrics() {
        guard systemSampler == nil else { return }
        systemSampler = SystemMetricsSampler()
        sampleSystemMetrics()
        ...
        $isExpanded.removeDuplicates().sink { [weak self] expanded in
            if expanded { self?.sampleSystemMetrics() }
        }.store(in: &cancellables)
        NotificationCenter.default.publisher(for: UserDefaults.didChangeNotification)
            .receive(on: RunLoop.main).sink { [weak self] _ in
                guard let self else { return }
                self.updateSnapshots(self.providerReadings)
            }.store(in: &cancellables)
    }

    private func sampleSystemMetrics() {
        systemMetrics = systemSampler?.sample() ?? []
        updateSnapshots(providerReadings)
    }
    ...
    func setLocalMetricsEnabled(_ enabled: Bool) {
        ...
        displaySnapshots = displaySnapshots.map(withPerformance)
    }
    ...
    func refresh(_ snapshot: ProviderSnapshot, using refreshProvider: (String) async -> Void) async {
        if systemMetric(for: snapshot) != nil { sampleSystemMetrics(); return }
    ...
    init() {
        // Language change leaves displaySnapshots untouched; tick `now` so copy
    ...
    var endSpread: CGFloat { endSpread(cellCount: displaySnapshots.count) }
    ...
    var bodyLength: CGFloat {
        NotchLayout.bodyLength(
            cellCount: displaySnapshots.count, edge: edge, spacing: cellSpacing
        ) + 2 * endSpread
    }
    ...
    var cellSpacing: CGFloat { cellSpacing(cellCount: displaySnapshots.count) }
    ...
    private func cellSpacing(cellCount: Int) -> CGFloat {
        ...
        let slack = NotchLayout.slack(for: edge,
            maxCardHeight: displaySnapshots.isEmpty ? NotchLayout.maxCardHeight(sessionCap: 0)
    ...
    var hoveredSnapshot: ProviderSnapshot? {
        guard let hoveredIndex, displaySnapshots.indices.contains(hoveredIndex) else { return nil }
        return displaySnapshots[hoveredIndex]
    }

    var shapeLength: CGFloat { shapeLength(cellCount: displaySnapshots.count) }

    var panelSize: CGSize { panelSize(cellCount: displaySnapshots.count) }
    ...
    var slack: CGFloat { slack(cellCount: displaySnapshots.count) }
    ...
    var sessionCap: Int { sessionCap(cellCount: displaySnapshots.count) }

    private var hasTokenUsage: Bool {
        displaySnapshots.contains { $0.tokenUsage != nil }
    }
    ...
    func maxCardHeight(cellCount: Int) -> CGFloat {
        ...
        return displaySnapshots.isEmpty
    ...
    /// Sized from an explicit count rather than from `displaySnapshots`.
    ///
