...
struct SettingsView: View {
    ...
    private var sidebar: some View {
        List(SettingsSection.allCases.filter { $0 != .ollama }, selection: $selection) { section in
            Label {
                Text(section.title)
            } icon: {
    ...
    private func paneContent(for section: SettingsSection) -> some View {
        ...
        case .ollama:
            Text("Unavailable in this Codex-only build.")
    ...
    private var accountsPane: some View {
        Form {
            ...
            Section(L10n.t("Connected")) {
                ...
                // the page.
                Text(L10n.t("Only Codex is connected. Credentials are read from Codex and never changed by this app."))
                    .font(.caption)
                    .foregroundStyle(.tertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        Form {
            ...
            Section {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 10) {
                    ...
                    // hiding.
                    Text(L10n.t("Local Codex-only build. Updates are disabled."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    Spacer(minLength: 0)
    ...
    static var setupCopy: String {
        L10n.t("Only Codex is connected. Credentials are read from Codex and never changed by this app.")
    }
