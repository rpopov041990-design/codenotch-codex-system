...
final class Updater: NSObject, ObservableObject, SPUUpdaterDelegate {
    ...
    var automatic: Bool {
        get { false }
        set { /* Codex-only builds must never replace themselves with upstream. */ }
    }
    ...
    /// broken button.
    func checkNow() {
        outcome = .failed("Codex-only local build: automatic and manual upstream updates are disabled.")
    }
